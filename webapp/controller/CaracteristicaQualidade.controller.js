	...
	return Controller.extend("br.com.idxtecCaracteristicaQualidade.controller.CaracteristicaQualidade", {
		onInit: function(){
			...
			this.getModel().attachMetadataLoaded(function(){
				...
				var oTable = oView.byId("tableCaracteristica");
				var oColumn = oView.byId("columnDescricao");
				
				oTable.sort(oColumn);
				oView.byId("tableCaracteristica").getBinding("rows").filter(oFilter, "Application");
			});
		},
		
		filtraCaract: function(oEvent){
			...
			this.getView().byId("tableCaracteristica").getBinding("rows").filter(aFilters, "Application");
		},
